...
class Normalizer:
    ...
    Input:
        List(tuple(float, float, float) | UnNormalized 21 Hand landmarks | X Y Z
    ...
    Output:
        List of 63 floats (Hand landmarks)
    """
    ...
    #Validation landmark if True, have 21 list, 3 coordinate each tuple and if it's a list.
    def validate(self, landmark: List[tuple[float, float, float]]) -> None:
        ...
        if not len(landmark) == 21:
            raise ValueError(f"Expected 21 tuples of landmarks, but got {len(landmark)}.")
        if not all(isinstance(point, tuple) and len(point) == 3 for point in landmark):
            raise ValueError('Expected landmark must be a tuple of 3 coordinates.')
        if not all(isinstance(coord, (int, float)) for point in landmark for coord in point):
            raise TypeError("Coordinates must be integer or float")
  
    #Translate landmarks relative to wrist. | Return: relative_landmarks
    def trans_relative_wrist(self, raw_landmark: List[tuple[float, float, float]]) -> List[float]:
        """
        Why:
            To make the wrist as point of origin to avoid in accurate reading of data.
        """
        x_wrist, y_wrist, z_wrist = raw_landmark[self.WRIST_INDEX] #Get the coordinates of WRIST.
        relative_coordinates = [] #Container of Coordinate relative to wrist.

        for point in raw_landmark: #landmark: [(2.0,5.0,6.9),(8.4,7.5,6.6), ......] -> point: (2.0, 5.0, 6.9)
    ...
    def transform(self, landmark: List[tuple[float, float, float]]) -> List[float]:
        
        #Validation the landmark
        self.validate(landmark)

        relative_landmarks = self.trans_relative_wrist(landmark) #Landmarks -> Relative Landmarks
